...
class Fe108(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        # print(' ------------------ length :{} self.sequence_list :{} ---------------------- '.format(len(self.sequence_list),self.sequence_list))
        # # seq_id is the index of the folder inside the got10k root path
        # if split is not None:
        #     if seq_ids is not None:
        #         raise ValueError('Cannot set both split_name and seq_ids.')
        #     if split == 'train':
        #         file_path = os.path.join('/data/zhu_19/FE108/FE108_inform/', 'train.txt')
        #     elif split == 'val':
        #         file_path = os.path.join('/data/zhu_19/FE108/FE108_inform/', 'test.txt')
        #     else:
        ...
        if split == 'train':
            name_ = os.path.join('/data/zhu_19/FE108/FE108_inform/', 'train.txt')
        elif split == 'val':
            name_ = os.path.join('/data/zhu_19/FE108/FE108_inform/', 'test.txt')
        self.sequence_list = self._get_sequence_list(name_)
    def get_name(self):
        return 'fe108'

    # def _get_sequence_list(self,name):
        # with open(os.path.join('/data/zhu_19/FE108/FE108_inform/', 'train.txt')) as f:
    ...
    def _get_frame_path(self, seq_path, frame_id):
        if os.path.exists(os.path.join(seq_path, '{:04}.png'.format(frame_id+1))):
            return os.path.join(seq_path, '{:04}.png'.format(frame_id+1))    # frames start from 0
        else:
            return os.path.join(seq_path, '{:04}.jpg'.format(frame_id+1))    # some image is bmp
    ...
    def _get_event_frame(self, seq_path, frame_id):
        ...
        for f_id in frame_id:
            ...
            # print('split of the seq paths are:{}'.format(seq_path_split))
            seq_path = os.path.join('/'+seq_path_split[1], seq_path_split[2], seq_path_split[3], 'FE108_inform', seq_path_split[5], seq_path_split[6]  )
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        return frame_list, anno_frames, object_meta, frame_event_list, None     # frame_event_img_list
