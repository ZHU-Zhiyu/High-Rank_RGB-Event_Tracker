...
def load_checkpoint(config, model, optimizer=None, lr_scheduler=None, loss_scaler=None, logger=None):
    ...
    pred_dict = checkpoint['model']
    print('name of checkpoint:{}'.format(checkpoint.keys()))
    non_load = []
    loaded = []
    for k, v in pred_dict.items():
        ...
        #     v.requires_grad_()
        # except:
        #     print('connot load:{}'.format(name))
        if name in model.state_dict() and v.shape == model.state_dict()[name].shape:
            new_state_dict[name] = v
            loaded.append(name)
            loaded.append(v.requires_grad)
        else:
            non_load.append(name)

    # print('------------not loaded model names are :{}'.format(non_load))
    # print('------------    loaded model names are :{}'.format(loaded))
    ...
    #     except:
    #         print('connot load:{}'.format(name))
    #     new_model.append(v.requires_grad)
    model.train()
    ...
    #     name = k # remove `module.`
    #     new_model.append(v.requires_grad)
    # print('------------    loaded model names are :{}'.format(new_model))
